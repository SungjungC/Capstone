...
class Db:
    ...
    def insertTable(self, email, nickname, password):  # 가입할때 정보 삽입
        ...
        self.connection.execute("INSERT INTO EX_MEMBER (`MEM_EMAIL`, `MEM_NICKNAME`, `MEM_PASSWORD`) VALUES(?,?,?)",
                                (email, nickname, password))
        self.connection.commit()

    def signupCheck(self, email):  # 가입할때 email 중복체크
        check = self.connection.execute("SELECT * FROM EX_MEMBER WHERE MEM_EMAIL = ?", [email])
    ...
    def loginCheck(self, nickname, password):  # 로그인할 때 확인
        result = self.connection.execute("SELECT * FROM EX_MEMBER WHERE MEM_NICKNAME = ? AND MEM_PASSWORD = ?",
                                         (nickname, password))
